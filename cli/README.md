...
email/LinkedIn ставь `--length long`.

**Двухэтапный пайплайн** (plan → write):

1. **Анализ (Hiring Manager).** LLM разбирает вакансию и строит стратегию: требования
   и их покрытие опытом, приоритизированные *talking points*, что подчеркнуть / чего
   избегать / честные пробелы. Только JSON, без текста письма. Сохраняется рядом как
   `out/cover-letters/<slug>-<lang>.plan.json` — можно посмотреть, ПОЧЕМУ выбраны блоки.
2. **Написание.** Второй LLM получает план и **только отобранные блоки опыта** (не весь
   каталог) — пересказать резюме физически нечем. Между этапами id блоков валидируются
   по каталогу (защита от галлюцинаций).

Если этап 1 упал/refusal — автоматический откат на одностадийную генерацию (в выводе
пометка `fallback`), чтобы результат был всегда. Письмо строго по фактам резюме
(`usedFacts` + `paragraphMap` — какой абзац какой тезис закрывает). Это черновик —
вычитай перед отправкой.
